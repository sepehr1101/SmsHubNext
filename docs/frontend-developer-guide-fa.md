...
هیچ Message Type یا Providerای هنگام migration ساخته نمی‌شود؛ ویزارد باید ابتدا آن‌ها را ثبت کند و شناسه‌های برگشتی را در مراحل بعد استفاده کند. اعداد نمونه این بخش صرفاً نمایشی‌اند و نباید به‌صورت ثابت در فرانت‌اند قرار گیرند. در محیط‌های `Development` و `Testing`، اگر provider واقعی فعال نباشد بک‌اند از `LoopbackSmsProvider` استفاده می‌کند؛ اما در محیط Production نبودن provider واقعی باعث خطای واضح هنگام شروع برنامه می‌شود تا ارسال آزمایشی با ارسال واقعی اشتباه نشود.
...
این نمونه فقط در محیط‌های `Development` و `Testing` برای تست مسیر `LoopbackSmsProvider` کافی است. اگر قرار است پیامک واقعی ارسال شود، ابتدا provider account واقعی ساخته و به sender line متصل شود و provider مربوطه نیز در تنظیمات برنامه فعال باشد.
