...
| App architecture | **Monolith organized by feature** (DntSite-inspired coding style; **not** Clean/Onion/CQRS/Vertical-Slice/MediatR/FluentValidation), minimal APIs, Dapper (no repo/UoW), plain in-feature validation, Result pattern, no Aspire | See `ARCHITECTURE.md`; providers behind `ISmsProvider`; background impl-agnostic (Hangfire candidate) |
