...
public sealed class ApiKeyAuthenticatorTests : IAsyncLifetime
{
    ...
    {
        var customerId = await CreateCustomerAsync("auth-ok");
        var issued = await IssueKeyAsync(customerId);

        var result = await new ApiKeyAuthenticator(_db)
    ...
    {
        var customerId = await CreateCustomerAsync("auth-expired");
        var issued = await IssueKeyAsync(customerId, expiresAtUtc: DateTime.UtcNow.AddDays(-1));
    ...
    {
        var key = await new IssueApiKeyHandler(_db).Handle(
