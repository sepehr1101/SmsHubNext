...
| **Provider Account** | A provider-specific sending account. Stores `AuthType`, non-sensitive `SettingsJson`, and one encrypted sensitive secret; sender lines point to the account that owns them. |
...
| 17 | `ProviderAccount` | Provider account auth settings + encrypted secret (security/infra) | <10 | <20 | <50 |
...
### 4.17 `ProviderAccount` — provider account auth data (security/infra)
| Column | Type | Notes |
|---|---|---|
| `Id` | `INT IDENTITY` | **PK**, `CIX` |
| `ProviderId` | `TINYINT` | **FK** → `Provider` |
| `DisplayName` | `NVARCHAR(100)` | human-readable account name |
| `AuthType` | `VARCHAR(50)` | e.g. `UsernamePasswordDomain`, `ApiKey` |
| `SettingsJson` | `NVARCHAR(MAX)` | non-sensitive provider-specific settings; JSON object |
| `SecretEncrypted` | `VARBINARY(MAX)` | encrypted sensitive secret — **ciphertext only** |
